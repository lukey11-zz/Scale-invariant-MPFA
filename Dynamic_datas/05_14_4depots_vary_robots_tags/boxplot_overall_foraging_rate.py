...
def compute_overall_forage_data(datas):
    ...
    for line in datas[1:]:
        ...
        forage.append(60.0*float(words[2]))
    return forage
    #mean = np.mean(forage)
    #std = np.std(forage)
    #return mean, std
...
fileNames = ["random_dynamic_MPFA_n4_r6_tag128_5by5_iAntTagData.txt", "random_dynamic_MPFA_n4_r12_tag512_10by10_iAntTagData.txt", "random_dynamic_MPFA_n4_r24_tag2048_20by20_iAntTagData.txt"]
without_comm_datas = get_multiple_data(fileNames)

avg_forage =[]
num_robots = [6.0,12.0, 24.0]
#pdb.set_trace()
...
axarr.set_xticklabels( ('6', '12', '24'), fontsize=18)
axarr.set_xlabel('Number of robots', fontsize=20)
axarr.set_yticks(np.arange(60, 160, 10))

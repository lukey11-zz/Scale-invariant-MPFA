        ...
        if (isHoldingFood) { 
	      num_targets_collected++;
	          ...
	          Pheromone sharedPheromone(SiteFidelityPosition, TrailToShare, timeInSeconds, LoopFunctions->RateOfPheromoneDecay, ResourceDensity);
                  //ClosestNest->PheromoneList.push_back(sharedPheromone);
                  LoopFunctions->PheromoneList.push_back(sharedPheromone);//qilu 03/29/2017
...
bool MPFA_controller::SetTargetPheromone() {
 ...
 if(LoopFunctions->PheromoneList.size()==0) return isPheromoneSet; //qilu 03/29/2017 the case of no pheromone.

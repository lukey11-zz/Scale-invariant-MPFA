...
//for(size_t n=0; n<loopFunctions.Nests.size(); n++){
	    for(size_t i = 0; i < loopFunctions.PheromoneList.size(); i++) {
             ...
             //for(j = 1; j < trail.size(); j++) {
                 //ray = CRay3(CVector3(trail[j - 1].GetX(), trail[j - 1].GetY(), 0.01), CVector3(loopFunctions.Nests[0].GetLocation().GetX(), loopFunctions.Nests[0].GetLocation().GetY(), 0.01));
                 ray = CRay3(CVector3(trail[0].GetX(), trail[0].GetY(), 0.01),
                 CVector3(trail[1].GetX(), trail[1].GetY(), 0.01));
